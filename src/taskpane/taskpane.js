...
export async function run() {
  try {
    ...
    await Excel.run(async context => {
      ...
      const LEVEL_MIN_THRESHOLD = 60;
      const M1_THRESHOLD = 60;
      const M2_THRESHOLD = 80;
      ...
      for (var i = 0; i < projectResponses.values.length; i++) {
        var level1CalculationDetails = CalculateLevelScore(projectResponses.values[i], responseScores, level1MaturityIndexes, 60);
        ...
        var maturity = "";
        if (finalScore <= M1_THRESHOLD) {
          maturity = "M1";
        }
        else if (finalScore > M1_THRESHOLD && finalScore <= M2_THRESHOLD) {
          maturity = "M2";
        }
        else if (finalScore > M2_THRESHOLD && finalScore <= M3_THRESHOLD) {
          maturity = "M3";
        }
        else if (finalScore > M3_THRESHOLD) {
          maturity = "M4";
        }
        ...
        //Highlighting Level Score if below threshold
